...
def create_projector(device, input_dim, output_dim, random_projector=True):
    ...
    Returns:
    - fixed_projector (nn.Linear): A linear layer with fixed weights and bias.
    """
    
    fixed_projector = nn.Linear(input_dim, output_dim)
    fixed_projector.to(device)
    
    with torch.no_grad():
        if random_projector:
            fixed_projector.weight.copy_(torch.randn(output_dim, input_dim))
            fixed_projector.bias.copy_(torch.randn(output_dim))
        elif output_dim > 1:
            fixed_projector.weight.copy_(torch.eye(output_dim, input_dim))
            fixed_projector.bias.fill_(1.0)
        else:
            weight = torch.zeros((output_dim, input_dim))
            weight[-1, -1] = 1
            fixed_projector.weight.copy_(weight)
            fixed_projector.bias.copy_(torch.tensor([-0.5]))

    fixed_projector.weight.requires_grad = False
    fixed_projector.bias.requires_grad = False
    
    return fixed_projector
