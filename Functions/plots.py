...
def levelsets(model, ax=None, path=None, fig_name=None, footnote=None, 
              contour=True, bar=True, plotlim=[-2, 2], step=0.01, dpi=100, 
    ...
    if contour:
        if preds.dim() == 3:
            colors = [to_rgb("C1"), [1, 1, 1], to_rgb("C0")]  # Gradient from Orange to Blue
    ...
    # Plot points
    point_size = 16
    ax.scatter(X0[:, 0], X0[:, 1], marker='X', edgecolor="#333", alpha=0.5, s=point_size)
    ax.scatter(X1[:, 0], X1[:, 1], marker='o', edgecolor="#333", alpha=0.5, s=point_size)
    ax.set_aspect('equal')
...
def loss_evolution(trainer, epoch, path, filename='', figsize=(8, 6), footnote=None, showl2norm=False, showgradnorm=False):
    """
...
def plot_data(model, inputs, targets, N, dpi=200, alpha=0.75, path='', trajs=False, init=False, final=False):
    """
...
def plot_loss(trainer, full_path, export_fig=False):
    """
